...
var SENSITIVE_KEYS = /^(?:access_token|refresh_token|client_secret|code|code_verifier|authorization|cookie|session_secret|supabase_secret_key|apikey)$/i;
function redactText(value) {
  return value.replace(/Bearer\s+[A-Za-z0-9._~+\/-]+/gi, "[REDACTED_AUTHORIZATION]").replace(/\bAPP_USR-[A-Za-z0-9-]+\b/g, "[REDACTED_ACCESS_TOKEN]").replace(/\bTG-[A-Za-z0-9-]+\b/g, "[REDACTED_REFRESH_TOKEN]").replace(/\bsb_secret_[A-Za-z0-9._-]+\b/gi, "[REDACTED_SUPABASE_SECRET]").replace(/postgres(?:ql)?:\/\/[^\s]+/gi, "[REDACTED_DATABASE_URL]").replace(/((?:access_token|refresh_token|client_secret|code_verifier|session_secret|supabase_secret_key|apikey)["'=:\s]+)[^\s&,}\"]+/gi, "$1[REDACTED]");
}
