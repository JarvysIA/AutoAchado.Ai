...
export async function outboxAction(client:SupabaseClient,action:string,b:any,origin:string,generation?:string) {
 ...
 if(action==='prepare') {
  ...
  const fresh=b.vertical==='APPLIANCES'?await (await import('../commercial/appliances-service.js')).revalidateAppliances(client,b.id,b.type):b.vertical==='HOME'?await (await import('../commercial/home-service.js')).revalidateHome(client,b.id,b.type):await revalidateProduct(client,b.id,b.type);
 ...
 if(action==='claim') {
  ...
  try {
   const fresh=job.vertical_key==='APPLIANCES'?await (await import('../commercial/appliances-service.js')).revalidateAppliances(client,job.product_id,job.product_type):job.vertical_key==='HOME'?await (await import('../commercial/home-service.js')).revalidateHome(client,job.product_id,job.product_type):await revalidateProduct(client,job.product_id,job.product_type);
