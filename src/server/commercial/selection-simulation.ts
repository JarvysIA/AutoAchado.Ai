...
export interface SelectionInputs {
 ...
 categories?:{marketplace_category_id:string;external_category_id:string}[];
}
// One bounded database snapshot avoids dozens of sequential HTTP requests.
export async function runSelectionSimulation(client:SupabaseClient,now=Date.now(),evaluationLimit=500,includeRaw=false) {
 ...
 // Snapshot categories are registry uuids; the family map is keyed by the official MLB id.
 const categories=await client.rpc('automotive_category_map');
 if(categories.error) throw new Error('SELECTION_CATEGORY_MAP_UNAVAILABLE');
 return analyzeSelectionInputs({...data,categories:Array.isArray(categories.data)?categories.data:[]},now,evaluationLimit,includeRaw);
}
// includeRaw keeps the scored candidates for the rebalance planner; the HTTP payload never does.
export function analyzeSelectionInputs(input:SelectionInputs,now=Date.now(),evaluationLimit=500,includeRaw=false) {
 const {snapshots,watches,memberships,feedback,changes}=input;
 // Snapshots carry the registry uuid; family and type need the official MLB id.
 const externalById=new Map((Array.isArray(input.categories)?input.categories:[]).map(c=>[c.marketplace_category_id,c.external_category_id]));
 const ranks=new Map<string,RankSignal[]>();
 for(const r of snapshots) {
  const key=r.type+':'+r.product_id,list=ranks.get(key)??[];
  list.push({category:externalById.get(r.marketplace_category_id)??r.marketplace_category_id,observed_at:r.observed_at,position:r.position});ranks.set(key,list);
 }
 ...
 const candidates:SelectionCandidate[]=[...new Set([...ranks.keys(),...memberships.map(m=>m.source_key)])].map(key=>{
  ...
  return {source_key:key,identity_key:identity,preview:w?.preview,category_id:w?.category_id??null,monitor:active.has(identity),protected:protectedIds.has(identity),
   feedback:actions.get(identity),monitor_since:memberships.find(m=>m.identity_key===identity&&m.monitor)?.monitor_since??monitoringStarts.get(key),ranks:ranks.get(key)??[]};
 });
 ...
 const summary=(c:typeof result.evaluated[number])=>({source_key:c.source_key,identity_key:c.identity_key,title:c.preview?.title??c.source_key,
  monitor:c.monitor,protected:c.protected,price_checked_at:c.preview?.priceCheckedAt??null,score:c.score,components:c.components,family:c.family,
  category_id:c.category_id,brand:c.brand,duplicate_key:c.duplicate_key,reason_codes:c.reasonCodes,demand:c.demand,confidence:c.confidence,
