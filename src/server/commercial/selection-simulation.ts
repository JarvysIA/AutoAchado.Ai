...
export async function runSelectionSimulation(client:SupabaseClient,now=Date.now(),evaluationLimit=500) {
...
export function analyzeSelectionInputs(input:SelectionInputs,now=Date.now(),evaluationLimit=500) {
 ...
 const summary=(c:typeof result.evaluated[number])=>({source_key:c.source_key,identity_key:c.identity_key,title:c.preview?.title??c.source_key,
  monitor:c.monitor,protected:c.protected,price_checked_at:c.preview?.priceCheckedAt??null,score:c.score,components:c.components,family:c.family,demand:c.demand,confidence:c.confidence,
