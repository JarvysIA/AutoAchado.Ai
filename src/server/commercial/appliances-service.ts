...
import {assessAppliances,resolveAppliancePreview,type AppliancePreview} from './appliances-editorial.js';
...
function checked<T>(r:{data:T;error:unknown}):T {if(r.error) throw new Error('APPLIANCES_STORAGE_UNAVAILABLE');return r.data;}
const identity=(id:string)=>'catalog:'+id+':new:BRL:public';
export function appliancesContext(title:string,category:string) {
...
export function appliancesEligible(p:AppliancePreview,category:string) {
 return p.appliance_specs?.verified===true&&assessAppliances(p.title,category).state==='CANDIDATE'&&p.status!=='UNAVAILABLE'&&p.comparable===true&&p.seller_trusted===true
...
async function enabled(client:SupabaseClient) {
 const row=checked(await client.from('commercial_verticals').select('enabled,executor_ready').eq('vertical_key','APPLIANCES').single());
...
async function history(client:SupabaseClient,ids:string[]) {
 ...
 for(let i=0;i<ids.length;i+=100){const current=checked(await client.from('appliances_candidates').select('identity_key,preview').in('identity_key',ids.slice(i,i+100)))??[];for(const row of current)if(row.preview?.appliance_specs?.variant_key)variants.set(row.identity_key,row.preview.appliance_specs.variant_key);}
 ...
 for(let startIndex=0;startIndex<ids.length;startIndex+=100) for(const table of ['appliances_observations','appliances_rank_observations']) for(let page=0;;page++) {
  const rows=checked(await client.from(table).select('*').in('identity_key',ids.slice(startIndex,startIndex+100)).gte('observed_at',start)
   .order('observed_at').order('identity_key').order(table==='appliances_observations'?'source_key':'category_id').range(page*1000,page*1000+999));
  result.push(...(rows??[]).filter(r=>table!=='appliances_observations'||(!!r.variant_key&&r.variant_key===variants.get(r.identity_key))).map(r=>table==='appliances_observations'?{...r,price:r.price===null?null:Number(r.price),position:null}:
   {...r,price:null,currency:'BRL',seller_id:null,comparable:false,trusted:false,demand_category:r.category_id}));
  if(!rows||rows.length<1000)break;if(page>=99)throw new Error('APPLIANCES_HISTORY_LIMIT');
 }return result;
}
async function savePrice(client:SupabaseClient,id:string,source:string,p:AppliancePreview) {
 ...
 checked(await client.from('appliances_observations').upsert({identity_key:identity(id),source_key:source,observed_at:p.priceCheckedAt,
  variant_key:p.appliance_specs.variant_key,price:p.price,currency:p.currency,seller_id:p.seller_id??null,comparable:true,trusted:p.seller_trusted===true},{onConflict:'identity_key,source_key,observed_at',ignoreDuplicates:true}));
}
async function observe(client:SupabaseClient,id:string,category:string,read:PreviewReader,p:AppliancePreview) {
...
async function assessStored(client:SupabaseClient,rows:any[]) {
 ...
 while(pending.length) {
  ...
  checked(await client.from('appliances_candidates').update({score,assessment:{...e,eligible:appliancesEligible(row.preview,row.category_id)&&demand.days>0,
...
async function allCandidates(client:SupabaseClient) {
 const rows:any[]=[];for(let page=0;;page++) {
  const batch=checked(await client.from('appliances_candidates').select('*').order('source_key').range(page*500,page*500+499));rows.push(...(batch??[]));
  if(!batch||batch.length<500)break;if(page>=19)throw new Error('APPLIANCES_CANDIDATE_LIMIT');
 }return rows;
}
export async function runAppliances(client:SupabaseClient,kind:'DISCOVERY'|'HISTORY') {
 await enabled(client);const run=checked(await client.rpc('begin_appliances_run',{run_kind:kind}));
 ...
 try {
  ...
  if(kind==='DISCOVERY') {
   const checks=checked(await client.from('appliances_category_checks').select('*'))??[];
   ...
   for(const category of categories) {
    ...
    try {
     ...
     if(c.id!==category.id||!c.path_from_root?.some((p:any)=>p.id==='MLB5726'))throw new Error('APPLIANCES_ANCESTRY');
     ...
     while(pending.length) {
      ...
      checked(await client.from('appliances_rank_observations').upsert({identity_key:identity(entry.id),category_id:category.id,position:entry.position,observed_at:new Date().toISOString()}));
      ...
      checked(await client.from('appliances_candidates').upsert({source_key:'PRODUCT:'+entry.id,product_id:entry.id,identity_key:identity(entry.id),category_id:category.id,family:category.family,preview:p},
       {onConflict:'source_key',ignoreDuplicates:true}));
      // Preserve established category, feedback and monitoring decisions.
      checked(await client.from('appliances_candidates').update({preview:p}).eq('source_key','PRODUCT:'+entry.id));
     ...
     if(complete)checked(await client.from('appliances_category_checks').upsert({category_id:category.id,checked_at:new Date().toISOString(),status:200}));
    }catch {failed++;checked(await client.from('appliances_category_checks').upsert({category_id:category.id,checked_at:new Date().toISOString(),status:503}));}
   }
  }else {
   const rows=checked(await client.from('appliances_candidates').select('*').eq('monitor',true).lte('next_check',new Date().toISOString()).order('next_check').order('source_key').limit(25))??[];
   const rankings=new Map<string,any[]>();
   for(const row of rows) {
    if(Date.now()>=deadline)break;
    checked(await client.from('appliances_candidates').update({last_attempt:new Date().toISOString(),next_check:new Date(Date.now()+3600000).toISOString()}).eq('source_key',row.source_key));
    try {
     ...
     if(rank&&Number.isInteger(rank.position)&&rank.position>=1&&rank.position<=20)checked(await client.from('appliances_rank_observations').upsert({identity_key:row.identity_key,category_id:row.category_id,position:rank.position,observed_at:new Date().toISOString()}));
     checked(await client.from('appliances_candidates').update({preview:p,next_check:new Date(Date.now()+(appliancesEligible(p,row.category_id)?12:1)*3600000).toISOString()}).eq('source_key',row.source_key));collected++;
    }catch {failed++;}
   }
  }
  await assessStored(client,await allCandidates(client));
  const admitted=checked(await client.rpc('renew_appliances_candidates'));
  checked(await client.from('appliances_runs').update({status:'COMPLETED',finished_at:new Date().toISOString(),collected,failed}).eq('id',run));
  return {status:'COMPLETED',collected,failed,admitted};
 }catch(error){await client.from('appliances_runs').update({status:'FAILED',finished_at:new Date().toISOString(),collected,failed}).eq('id',run);throw error;}
}

export async function appliancesOpportunities(client:SupabaseClient,view:string,offset:number) {
 await enabled(client);const rows=await allCandidates(client);
 const sent=checked(await client.from('commercial_sent_products').select('identity_key,sent_at').eq('vertical_key','APPLIANCES'))??[];
 ...
 const entries=watches.map(w=>{
  ...
  return {identity_key:w.identity_key,monitor:w.monitor,feedback:w.feedback,sent_at:sent.find(s=>s.identity_key===w.identity_key)?.sent_at??null,
   snapshot:{product_id:w.product_id,type:'PRODUCT',category_id:w.category_id,vertical_key:'APPLIANCES'},preview:{...p,...affiliateIntelligence(p)},
   ...
   rank:rankProduct(p,own,w.feedback,Date.now(),appliancesContext(p.title,w.category_id)),price_analysis:analyzePriceTruth(p,own),price_timeline:priceTimeline(p,own)};
 });
 ...
 const runs=checked(await client.from('appliances_runs').select('*').order('started_at',{ascending:false}).limit(1));
...
export async function revalidateAppliances(client:SupabaseClient,id:string,type:string) {
 await enabled(client);if(type!=='PRODUCT')throw new Error('APPLIANCES_PRODUCT_TYPE');
 const row=checked(await client.from('appliances_candidates').select('*').eq('product_id',id).maybeSingle());
 ...
 checked(await client.from('appliances_candidates').update({preview:p}).eq('product_id',id));
 const own=await history(client,[row.identity_key]);
 return {ready:p.appliance_specs?.verified===true&&p.status!=='UNAVAILABLE'&&!!p.title&&p.title!==id&&!!p.price&&p.currency==='BRL'&&!!safePreviewUrl(p.image,true)&&!!safePreviewUrl(p.url)&&Date.parse(p.priceCheckedAt??'')>=Date.now()-60000,
...
export async function appliancesAction(client:SupabaseClient,id:string,type:string,action:string,sent?:boolean) {
 await enabled(client);if(type!=='PRODUCT')throw new Error('APPLIANCES_PRODUCT_TYPE');
 const row=checked(await client.from('appliances_candidates').select('identity_key').eq('product_id',id).maybeSingle());if(!row)throw new Error('APPLIANCES_PRODUCT_NOT_FOUND');
 if(action==='sent')checked(await client.from('commercial_sent_products').upsert({vertical_key:'APPLIANCES',identity_key:row.identity_key,sent_at:sent?new Date().toISOString():null,updated_at:new Date().toISOString()}));
 else checked(await client.from('appliances_candidates').update({feedback:action,...(action==='NOT_RELEVANT'?{monitor:false}: {})}).eq('product_id',id));
 return {saved:true};
}
