...
const preview:ProductPreview={priceLinkVerified:true,title:'Mini aspirador portátil',description:'Uso automotivo',image:'https://http2.mlstatic.com/a.jpg',url:'https://www.mercadolivre.com.br/p/MLB123',price:80,currency:'BRL',status:'AVAILABLE',catalog_product_id:'MLB123',comparable:true,seller_trusted:true,seller_id:'1',seller_level:'5_green',priceCheckedAt:new Date(now).toISOString()};
...
// Aspiradores: a família agora vem da categoria oficial, não do título.
const CATEGORY='MLB22723';
const ranked=(p:ProductPreview,h:Observation[],f:string|null=null,n=now,ctx:Record<string,unknown>={})=>rankProduct(p,h,f,n,{categoryId:CATEGORY,...ctx});
describe('commercial gates before scoring',()=>{
 it('never combines days across different ranking dimensions',()=>{
  ...
  expect(ranked(preview,[...rows,...ranks],null,now)).toMatchObject({state:'OBSERVING',demand_days:6});
 });
 it('approves independent history, recurring demand and trusted current offer',()=>{
  expect(ranked(preview,history(),null,now)).toMatchObject({state:'APPROVED',historical_discount_percent:20,reference_price:100,history_days:28,seller_count:2});
 });
 it('never lets a huge discount replace demand',()=>{
  const rank=ranked({...preview,price:1},history().map(o=>({...o,position:null})),null,now);
 ...
 it('does not manufacture history from repeated intraday observations',()=>{
  ...
  expect(ranked(preview,rows,null,now)).toMatchObject({state:'OBSERVING',history_days:1});
 });
 it('requires multi-seller coverage, even with a long seller history',()=>{
  expect(ranked(preview,history().map(o=>({...o,seller_id:'1'})),null,now).state).toBe('OBSERVING');
 });
 it('excludes today, foreign currency, untrusted and incomparable observations',()=>{
  ...
  expect(ranked(preview,invalid,null,now)).toMatchObject({state:'OBSERVING',history_days:0});
 });
 it('uses the daily minimum before taking the median and ignores seller original_price',()=>{
  ...
  expect(ranked({...preview,original_price:9999},rows,null,now).reference_price).toBe(100);
  expect(ranked({...preview,price:90.01},rows,null,now).state).toBe('REJECTED');
 });
 it('requires a fresh price and verified seller, and applies audience rejection',()=>{
  expect(ranked({...preview,priceCheckedAt:new Date(now-2*86400000).toISOString()},history(),null,now).state).toBe('OBSERVING');
  expect(ranked({...preview,seller_trusted:false},history(),null,now).state).toBe('OBSERVING');
  expect(ranked(preview,history(),'NOT_RELEVANT',now).state).toBe('REJECTED');
  expect(ranked({...preview,title:'Rastreador GPS com mensalidade'},history(),null,now).state).toBe('REJECTED');
 });
 it('deduplicates identities and limits each group to three approved products',()=>{
  const entries=Array.from({length:10},(_,i)=>({identity_key:'p'+Math.floor(i/2),rank:ranked(preview,history(),null,now)}));
