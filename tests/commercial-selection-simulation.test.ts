...
function fixture(fail=false) {
 ...
 const reads:string[]=[];
 const client={rpc:async(name:string)=>{reads.push(name);
  // O simulador traduz o uuid do registro para a categoria oficial antes de decidir a família.
  if(name==='automotive_category_map') return {data:[{marketplace_category_id:'category',external_category_id:'MLB63533'}],error:null};
  return {data:{snapshots:tables.highlight_snapshots,
...
it('simulates against persisted discovery data without writes or changing membership',async()=>{
 ...
 expect(reads).toEqual(['commercial_selection_inputs','automotive_category_map']);
});
