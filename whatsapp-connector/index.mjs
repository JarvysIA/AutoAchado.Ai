...
async function tick(){
 if(busy||!config||!client)return;busy=true;
 try {
...
async function start(){
 ...
 client.on('ready',()=>{connected=true;qr=null;status='WhatsApp conectado.';void tick();});
 client.on('disconnected',()=>{connected=false;status='WhatsApp desconectado. Reinicie o conector para reconectar.';});
 ...
 try{await client.initialize();}catch{status='Não foi possível iniciar o navegador. Confira a instalação e reinicie.';}finally{initializing=false;}
}
...
<h1>WhatsApp do Cyber Ofertas</h1><p id="status"></p><img id="qr" hidden alt="QR Code para conectar o WhatsApp"><section id="setup"><p>1. Na dashboard, abra <b>WhatsApp — conexão, grupos e envios</b> e baixe a configuração.</p><p>2. Selecione o arquivo aqui:</p><input id="file" type="file" accept="application/json"><button id="import">Importar configuração</button></section><p>3. No celular: WhatsApp → Configurações → Aparelhos conectados → Conectar aparelho.</p><p>Depois, escolha os grupos na dashboard. Somente as mensagens que você confirmar serão enviadas. Mantenha o notebook ligado e acordado.</p>
<script>async function refresh(){const r=await fetch('/status');const s=await r.json();document.getElementById('status').textContent=s.status;const img=document.getElementById('qr');img.hidden=!s.qr;if(s.qr)img.src=s.qr;}document.getElementById('import').onclick=async()=>{try{const f=document.getElementById('file').files[0];if(!f)return;const response=await fetch('/configure',{method:'POST',headers:{'Content-Type':'application/json','X-Cyber-Setup':'1'},body:await f.text()});if(!response.ok)throw Error();await refresh();}catch{document.getElementById('status').textContent='Configuração inválida ou conector já iniciado. Reinicie para trocar a configuração.';}};refresh();setInterval(refresh,2500);</script></html>`;
