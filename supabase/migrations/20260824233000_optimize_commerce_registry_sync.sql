create or replace function public.apply_commerce_registry_sync(p_payload jsonb)
returns jsonb
...
begin
  ...
  if exists (
    with input_rows as materialized (
      select * from private.commerce_registry_input_rows(r)
    ),
    node_name_map as materialized (
      select pg_catalog.jsonb_object_agg(external_category_id, pg_catalog.to_jsonb(name)) as names_by_id
      from input_rows
    )
    select 1
    from input_rows x
    cross join node_name_map nodes
    where pg_catalog.cardinality(path_external_ids)=0 or pg_catalog.cardinality(path_names)=0
      ...
      or exists (
        select 1
        from pg_catalog.generate_subscripts(path_external_ids,1) path(position)
        where (nodes.names_by_id ->> path_external_ids[path.position])
          is distinct from path_names[path.position])
...
comment on function public.apply_commerce_registry_sync(jsonb) is
  'Atomically applies a versioned commerce registry payload with set-based path validation and manual-decision protection.';
